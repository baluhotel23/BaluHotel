...
console.log("Wompi reference:", reference);
  ...
  reference: reference,
  publicKey: publicKey, // <-- usa la variable, no el string hardcodeado
  redirectUrl: redirectUrl,
  customerData: {
    email: booking.customer_email,
          // Consider adding more if available and required/recommended by Wompi:
          // fullName: booking.customer_name,
          // phoneNumber: booking.customer_phone,
          // legalId: booking.customer_document_number, // If applicable
          // legalIdType: booking.customer_document_type, // If applicable
        }
      };
      ...
      checkout.open((result) => {
        const { transaction, error } = result; // Wompi might return an error object in result
        if (transaction) {
          ...
          // Optionally, call onPaymentComplete with an error or a specific status
          // onPaymentComplete({ error: error || 'Widget closed without transaction' });
        }
      });
